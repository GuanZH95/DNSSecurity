...
* 基于文本内容识别赌博/色情网站         
    ...
    考虑到爬取执行的时间, 首先对550w个域名进行数据清洗, 包括:                                                                                    
		...
		去掉一些不指向具体网页的域名, 这类域名在请求中频繁出现, 包括**xx.netease.com**, 网易的DNS检测域名; **xx.irs01.com**, 艾瑞数据在视频等网站中记录用户行为使用的域名; **xx.imtmp.net**, 即时通讯软件向服务器拉取数据使用的域名; **xx.in-addr.arpa**, 反向PTR查询使用的域名等                                                                                                               
